...
exports.getUserinfo = (req, res) => {
    ...
    connectionUIRM.query(sqlStr, req.user.id, (err, result) => {
        if (err)
            return res.send({
                status: 1,
                message: err
...
exports.updateUserinfo = (req, res) => {
    ...
    connectionUIRM.query(sqlStr, [
        ...
        connectionUIRM.query('update fd_users set is_new_user=1 where id=?', [req.user.id], (err, result) => {
            if (err)
                return res.send({
                    status: 1,
                    message: err
...
exports.updateAvatar = (req, res) => {
    ...
    // 查询之前的文件名 然后删除
    connectionUIRM.query(deleteSql, [req.user.id], (err, result) => {
        console.log(result);
        if (err)
            return res.send({
                status: 1,
        ...
        if (fileName != null) {
            ...
            fsUIRM.unlinkSync(deleteFile, (err) => {
                if (err) {
                    console.error(err);
                    return;
                }
    ...
    connectionUIRM.query(sqlStr, [sqlAvatarUrl, req.file.filename, req.user.id], (err, result) => {
        if (err)
            return res.send({
                status: 1,
                message: err
