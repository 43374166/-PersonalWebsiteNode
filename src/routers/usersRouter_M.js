...
const login = (req, res) => {
  ...
  connection.query(sqlStr, [userinfo.username], (err, result) => {
    ...
    const tokenStr = jwt.sign(user, config.jwtSecretKey, {expiresIn: config.expiresIn}) // token有效时间为100天
...
const registerUser = (req, res) => {
  ...
  connection.query(sqlStr, [userinfo.username], (err, result) => {
    ...
    if(result.length > 0) {
      return res.send({status: 1, message: '已存在用户名！请更换'})
    }
    ...
    connection.query(intoSqlStr, {username: userinfo.username, password: userinfo.password, email: userinfo.email}, (err, result) => {
      if(err) {
        return res.send({status: 1, message: err.message})
      }
...
const sentemail = (req, res) => {
  ...
  connection.query(sqlStr, [email], (err, result) => {
    ...
    if(result.length > 0) {
      return res.send({status: 1, message: '该邮箱已经被注册! '})
    }
...
module.exports = {login, registerUser, sentemail}